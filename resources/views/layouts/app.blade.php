...
<head>
    ...
    <style>
        ...
        html { background-color: var(--mkt-bg); transition: none; }
        body { font-family: 'Inter', sans-serif; min-height: 100vh; background-color: var(--mkt-bg); }


        /* Utility classes that respond to theme */
        .mkt-bg         { background-color: var(--mkt-bg) !important; }
        .mkt-surface    { background-color: var(--mkt-surface) !important; }
        .mkt-surface-alt{ background-color: var(--mkt-surface-alt) !important; }
        .mkt-border     { border-color: var(--mkt-border) !important; }
        .mkt-text       { color: var(--mkt-text) !important; }
        .mkt-text-muted { color: var(--mkt-text-muted) !important; }
        .mkt-input      { background-color: var(--mkt-input-bg) !important; color: var(--mkt-text) !important; }
        ...
        *, *::before, *::after {
        ...
        .overflow-y-auto::-webkit-scrollbar-thumb:hover { background: #ef4444; }
    </style>
    @livewireStyles
</head>
    ...
    <div class="min-h-screen">
        ...
        <nav class="mkt-surface border-b mkt-border sticky top-0 z-50 transition-colors duration-300">
            @if(app()->isDownForMaintenance() && auth()->user()->isSuperAdmin())
                <div class="bg-amber-600/90 backdrop-blur-md text-white py-1.5 text-center text-[10px] font-black uppercase italic tracking-[0.3em] animate-pulse border-b border-amber-500/50">
            ...
            <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
                <div class="flex justify-between h-16 sticky top-0 z-50">
                    <div class="flex items-center">
                        ...
                        <div class="hidden xl:flex space-x-2 xl:ml-10 items-center">
                            ...
                            <div class="hidden 2xl:flex items-center ml-8 relative group">
                                <span class="absolute left-4 text-slate-500 group-focus-within:text-red-500 transition-colors">🔍</span>
                                <input type="text" 
                                    placeholder="GLOBAL SAP SEARCH..." 
                    ...
                    <div class="flex items-center gap-2">
                        {{-- Theme Toggle --}}
                        <div class="flex items-center">
                            <button @click="toggleTheme()" class="p-2 rounded-full mkt-surface-alt mkt-text-muted hover:mkt-text transition-all shadow-sm">
                                <template x-if="!isDark">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 3v1m0 16v1m9-9h-1m-16 0h-1m15.364-6.364l-.707.707M6.343 17.657l-.707.707M16.243 17.657l.707-.707M7.757 6.343l.707-.707M12 8a4 4 0 110 8 4 4 0 010-8z"></path></svg>
                                </template>
                                <template x-if="isDark">
                        ...
                        <div class="hidden xl:flex items-center gap-4 ml-2 pl-4 border-l mkt-border">
                            <div class="text-right">
                                <div class="text-[10px] font-black mkt-text uppercase tracking-widest italic">{{ Auth::user()->name }}</div>
                                <div class="text-[8px] mkt-text-muted font-bold uppercase">{{ Auth::user()->role }}</div>
                            </div>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="p-2 rounded-full mkt-surface-alt text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-sm">
                        ...
                        <div class="xl:hidden flex items-center" x-data="{ open: false }">
                            <button @click="$dispatch('open-menu')" class="p-2 rounded-xl mkt-surface-alt mkt-text-muted hover:mkt-text transition-all border mkt-border shadow-sm">
                ...
                <div class="absolute right-0 top-0 bottom-0 w-80 mkt-surface shadow-2xl transition-transform"
                    ...
                    <div class="p-6 h-full flex flex-col">
                        <div class="flex justify-between items-center mb-10">
                            <div class="text-lg font-black text-red-600 italic">MENU <span class="mkt-text">EXPLORER</span></div>
                        ...
                        <div class="flex-grow space-y-2 overflow-y-auto pr-2">
                            ...
                            <a href="{{ route('dashboard') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('dashboard') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">Dashboard</a>
                            
                            @if(in_array(auth()->user()->role, ['admin', 'super-admin']))
                                <a href="{{ route('admin.monitoring') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('admin.monitoring') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">Monitoring</a>
                                <a href="{{ route('admin.unit-monitoring') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('admin.unit-monitoring') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">Unit Monitoring</a>
                                <a href="{{ route('admin.activity-logs') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('admin.activity-logs') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">Audit Trail</a>
                                
                                @if(auth()->user()->role === 'super-admin')
                                    <a href="{{ route('admin.users') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('admin.users') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">Users Management</a>
                                    <a href="{{ route('admin.divisions') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('admin.divisions') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">Divisions</a>
                                    <a href="{{ route('admin.config') }}" class="flex items-center px-4 py-3 rounded-2xl {{ request()->routeIs('admin.config') ? 'bg-red-600 text-white' : 'mkt-text-muted hover:mkt-surface-alt' }} font-bold text-sm">System Config</a>
                                @endif
                            @endif
                        </div>
                        ...
                        <div class="mt-auto pt-6 border-t mkt-border">
                            <div class="flex items-center gap-3 mb-6">
                                <div class="w-10 h-10 rounded-2xl bg-red-600 flex items-center justify-center text-white font-black">
                                    {{ substr(Auth::user()->name, 0, 1) }}
                                </div>
                                <div>
                                    <div class="text-xs font-black mkt-text uppercase">{{ Auth::user()->name }}</div>
                                    <div class="text-[9px] mkt-text-muted font-bold uppercase">{{ Auth::user()->role }}</div>
                                </div>
                            </div>
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="w-full py-4 rounded-2xl bg-red-600/10 text-red-600 font-black uppercase text-xs tracking-widest hover:bg-red-600 hover:text-white transition-all">
                                    LOGOUT SYSTEM
                                </button>
                            </form>
                        </div>
