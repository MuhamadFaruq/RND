{{-- resources/views/livewire/operator/pengujian-form.blade.php --}}

<div x-data="{ showReq: false }">
        ...
        <div class="w-full max-w-[1600px] mx-auto px-3 sm:px-4 md:px-6">
            <div class="mkt-surface p-4 md:p-6 rounded-2xl shadow-sm border mkt-border">
                ...
                <div class="flex items-center gap-3 mb-6">
                    <div class="bg-cyan-500/80 p-3 rounded-2xl text-white shadow-lg shadow-cyan-600/20">
                                            </div>
                    <div>
                        <h2 class="text-base md:text-lg font-black uppercase tracking-tighter mkt-text">Pengujian (QC & LAB)</h2>
                        <p class="text-[10px] font-bold mkt-text-muted uppercase tracking-widest">Final Quality Control & Lab Test #{{ $order->art_no }}</p>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                    ...
                    <div class="mkt-surface-alt border mkt-border p-6 rounded-[2rem] italic overflow-y-auto max-h-[160px]">
                        <div class="flex items-center justify-between mb-3 border-b mkt-border pb-2">
                            <h4 class="text-[9px] font-black uppercase mkt-text-muted tracking-widest">History Produksi</h4>
                            <button @click="showReq = true" type="button" class="text-[8px] font-black text-cyan-500 uppercase hover:underline">Lihat Detail Spek</button>
                        </div>
                ...
                @if(!empty($pipelineErrors))
                    <div class="mb-6 bg-amber-950/50 backdrop-blur-md border-2 border-amber-500 rounded-2xl p-5 space-y-2">
                        <div class="flex items-center gap-2 mb-3">
                            ...
                            <p class="text-amber-400 font-black uppercase text-xs tracking-widest">Proses Produksi Belum Lengkap</p>
                        </div>
                        ...
                        @endforeach
                        <p class="text-indigo-600 text-[10px] font-black uppercase mt-3 pt-3 border-t border-amber-800">Pengujian tidak dapat dilanjutkan sampai semua proses di atas selesai.</p>
                    </div>
                @endif
                 ...
                 <form wire:submit.prevent="submit" class="space-y-5">
                    {{-- I. IDENTITAS & WAKTU --}}
                    <div class="space-y-4">
                        <p class="text-[9px] font-black text-indigo-500 uppercase tracking-[0.3em] border-l-4 border-indigo-500 pl-3">I. IDENTITAS & WAKTU (PENGUJIAN QC & LAB)</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white/5 p-6 rounded-3xl border mkt-border">
                            <div>
                                <label class="block text-[10px] font-black text-indigo-600 italic uppercase mb-2 ml-1">Operator Penguji</label>
                                <input type="text" wire:model="operator" placeholder="NAMA PENGUJI..."
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-indigo-600 italic uppercase mb-2 ml-1">Tanggal Uji</label>
                    ...
                    <div class="space-y-4">
                        <p class="text-[9px] font-black text-cyan-500 uppercase tracking-[0.3em] border-l-4 border-cyan-500 pl-3">II. HASIL PENGUJIAN FISIK</p>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white/5 p-6 rounded-3xl border mkt-border">
                            <div>
                                <label class="block text-[10px] font-black text-cyan-500 italic uppercase mb-2 ml-1">Hasil Lebar (cm)</label>
                                <input type="number" step="any" wire:model="lebar" placeholder="HASIL LEBAR..."
                            ...
                            <div>
                                <label class="block text-[10px] font-black text-cyan-500 italic uppercase mb-2 ml-1">Hasil Gramasi (gsm)</label>
                                <input type="number" step="any" wire:model="gramasi" placeholder="HASIL GRAMASI..."
                                    class="w-full mkt-surface border mkt-border rounded-xl px-4 py-3 font-black text-xs text-cyan-400 placeholder-slate-500 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 transition-all outline-none italic uppercase">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-white/5 p-6 rounded-3xl border mkt-border">
                            <div>
                                <label class="block text-[10px] font-black text-emerald-500 italic uppercase mb-2 ml-1">Shrinkage (%)</label>
                                <input type="number" step="any" wire:model="shrinkage" placeholder="SHRINKAGE..."
                                    class="w-full mkt-surface border mkt-border rounded-xl px-4 py-3 font-black text-xs text-emerald-400 placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all outline-none italic uppercase">
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-emerald-500 italic uppercase mb-2 ml-1">Spirality (%)</label>
                                <input type="number" step="any" wire:model="spirality" placeholder="SPIRALITY..."
                                    class="w-full mkt-surface border mkt-border rounded-xl px-4 py-3 font-black text-xs text-emerald-400 placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all outline-none italic uppercase">
                            </div>
                            <div>
                                <label class="block text-[10px] font-black text-emerald-500 italic uppercase mb-2 ml-1">Skewness (%)</label>
                                <input type="number" step="any" wire:model="skewness" placeholder="SKEWNESS..."
                                    class="w-full mkt-surface border mkt-border rounded-xl px-4 py-3 font-black text-xs text-emerald-400 placeholder-slate-500 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-all outline-none italic uppercase">
                            </div>
                        </div>
                    </div>
                    ...
                    <button type="submit"
                        class="w-full bg-cyan-600/80 backdrop-blur-md text-white py-4 rounded-xl font-black uppercase text-[10px] shadow-lg transition-all hover:bg-cyan-600 hover:scale-[1.01] group">
        ...
        <div class="mkt-surface w-full max-w-3xl rounded-2xl md:rounded-3xl border mkt-border shadow-2xl relative overflow-hidden italic flex flex-col max-h-[85vh]">
            ...
            <div class="px-5 md:px-8 pt-6 pb-4 border-b mkt-border mkt-surface-alt sticky top-0 z-10 flex items-center justify-between">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-cyan-600/80 rounded-2xl flex items-center justify-center text-white text-xl shadow-lg shadow-cyan-500/20"></div>
                    <div>
                ...
                <button @click="showReq = false" class="w-10 h-10 flex items-center justify-center rounded-xl bg-slate-800 text-slate-400 hover:bg-indigo-600 hover:text-white transition-all text-xl">✕</button>
            </div>
            ...
            <div class="px-10 py-4 mkt-surface-alt border-b mkt-border flex items-center gap-3 overflow-x-auto no-scrollbar">
                <button wire:click="$set('activeDetailTab', 'marketing')" 
                    class="flex-none px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all {{ $activeDetailTab === 'marketing' ? 'bg-cyan-600 text-white shadow-xl scale-105' : 'mkt-surface mkt-text hover:bg-slate-100' }}">
                    Marketing Req.
                </button>
                ...
                @if($activeDetailTab === 'marketing')
                    <div class="space-y-6 animate-in fade-in slide-in-from-bottom-2">
                        <div class="flex items-center gap-4 mb-4">
                            <div class="w-10 h-10 bg-cyan-600 rounded-xl flex items-center justify-center text-white text-lg"></div>
                            <div>
                                <h3 class="text-sm font-black uppercase mkt-text leading-none">Marketing Requirements</h3>
                                <p class="text-[9px] font-bold mkt-text-muted uppercase tracking-widest mt-1">Artikel #{{ $order->art_no }} Specs</p>
                            </div>
                        </div>
                        ...
                        <div class="p-6 mkt-surface-alt border mkt-border rounded-3xl relative overflow-hidden">
                            ...
                            @endif
                            <p class="text-[9px] font-black uppercase text-cyan-500 mb-2 tracking-[0.2em]">Customer / Brand</p>
                        ...
                        <div class="space-y-4 pt-4">
                            ...
                            <div>
                                <p class="text-[8px] font-black uppercase mkt-text-muted mb-1 tracking-widest">Instruksi Khusus (Treatment)</p>
                                <p class="text-[10px] font-bold mkt-text-muted uppercase leading-relaxed italic">{{ $order->treatment_khusus ?? 'Tidak ada instruksi khusus.' }}</p>
                            </div>
            ...
            <div class="px-5 md:px-8 py-5 mkt-surface border-t mkt-border flex justify-end">
                <button @click="showReq = false" class="px-8 py-3 bg-slate-800 text-white rounded-xl font-black uppercase text-[10px] tracking-wider hover:bg-indigo-600 hover:shadow-xl transition-all active:scale-95 border border-white/5">
                    Tutup Detail
                </button>
            </div>
        </div>
