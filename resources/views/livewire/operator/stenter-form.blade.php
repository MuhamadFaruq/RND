            ...
            <div class="bg-white rounded-[2.5rem] p-8 shadow-xl border border-slate-100">
                ...
                <div class="flex bg-slate-100 p-2 rounded-2xl mb-8">
                    ...
                    <button wire:click="$set('sub_proses', '{{ $tab }}')" 
                        class="flex-1 py-3 rounded-xl font-black uppercase text-[10px] transition-all {{ $sub_proses === $tab ? 'bg-indigo-600 text-white shadow-lg' : 'text-slate-400' }}">
                        {{ $tab }}
                    </button>
                    @endforeach
                </div>
                ...
                <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                        <h2 class="text-xl font-black uppercase tracking-tighter text-slate-800">Stenter (Finishing Belah)</h2>
                        <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Mode: <span class="text-indigo-600">{{ strtoupper($sub_proses) }}</span></p>
                    </div>
                </div>
                ...
                <form wire:submit.prevent="submit" class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="no_mesin" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold" placeholder="Contoh: ST-01">
                        </div>
                    </div>

                    <div class="grid grid-cols-2 md:grid-cols-4 gap-6 pt-4 border-t border-slate-100">
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Temperature (Int)</label>
                            <input type="number" wire:model="temperatur" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold text-indigo-600">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Speed (Int)</label>
                            <input type="number" wire:model="speed" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold text-indigo-600">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Padder (Int)</label>
                            <input type="number" wire:model="padder" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Rangka (Int)</label>
                            <input type="number" wire:model="rangka" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold">
                        </div>
                    </div>

                    <div class="grid grid-cols-2 md:grid-cols-3 gap-6">
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Overfeed A (Int)</label>
                            <input type="number" wire:model="overfeed_a" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Overfeed B (Int)</label>
                            <input type="number" wire:model="overfeed_b" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold">
                        </div>
                        <div class="col-span-2 md:col-span-1">
                            <label class="block text-[10px] font-black text-slate-400 uppercase mb-2 ml-1">Fan / Blower (Int)</label>
                            <input type="number" wire:model="fan_blower" class="w-full bg-slate-50 border-2 border-slate-100 rounded-xl px-4 py-3 font-bold text-orange-600">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-2 gap-6 bg-indigo-50/50 p-6 rounded-[2rem] border border-dashed border-indigo-200">
                        <div>
                            ...
                            <input type="number" wire:model="chemical_1" class="w-full bg-white border-2 border-indigo-100 rounded-xl px-4 py-3 font-bold">
                        </div>
                        <div>
                            <label class="block text-[10px] font-black text-indigo-400 uppercase mb-2 ml-1">Chemical 2 (Int)</label>
                            <input type="number" wire:model="chemical_2" class="w-full bg-white border-2 border-indigo-100 rounded-xl px-4 py-3 font-bold">
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 bg-slate-900 p-6 rounded-[2.5rem] shadow-xl">
                        ...
                        <div>
                            <label class="block text-[9px] font-black text-slate-400 uppercase mb-2 ml-1">Shrinkage (Int)</label>
