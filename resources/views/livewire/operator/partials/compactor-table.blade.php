    ...
    <table class="w-full text-left border-collapse">
        ...
        <tbody class="divide-y mkt-border italic">
            @forelse($workQueue as $job)
                <tr class="hover:bg-brand-600/5 transition-colors">
                    <td class="px-4 lg:px-5 py-3">
                        <span class="text-[10px] font-black mkt-text-muted uppercase">#{{ $job->art_no }}</span>
                        @if($job->is_urgent)
                    ...
                    <td class="px-4 lg:px-5 py-3 text-xs">
                        <span class="px-2 py-0.5 bg-brand-600/10 mkt-text-muted rounded-full text-[8px] font-black uppercase">
                            Siap Compactor
                        </span>
                    </td>
                    <td class="px-4 lg:px-5 py-3 text-center">
                        <button wire:click="showOrderDetail({{ $job->id }})" 
                                class="mkt-surface-alt border mkt-border mkt-text px-4 py-1.5 rounded-xl text-[9px] font-black uppercase hover:bg-brand-600 hover:text-white transition-all shadow-sm">
                            DETAIL & PROSES
                        </button>
                    </td>
                </tr>
        ...
        <div class="mkt-surface p-4 rounded-2xl shadow-sm border mkt-border italic">
            <div class="flex items-start justify-between mb-2">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        <span class="text-[9px] font-black mkt-text-muted uppercase">#{{ $job->art_no }}</span>
                        @if($job->is_urgent)
                            <span class="ml-1 bg-red-600 text-white text-[7px] font-black px-1.5 py-0.5 rounded-full uppercase animate-pulse">URGENT</span>
                        @endif
                ...
                <span class="px-2 py-0.5 bg-brand-600/10 mkt-text-muted rounded-full text-[7px] font-black uppercase shrink-0">Siap Compactor</span>
            </div>
            <div class="flex justify-end pt-2 border-t mkt-border">
                <button wire:click="showOrderDetail({{ $job->id }})" 
                    class="mkt-surface-alt border mkt-border mkt-text px-3 py-1.5 rounded-xl font-black text-[8px] uppercase hover:bg-brand-600 hover:text-white transition-all">DETAIL & PROSES</button>
            </div>
        </div>
    @empty
