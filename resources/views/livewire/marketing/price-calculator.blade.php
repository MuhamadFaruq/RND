<div x-data="{
...
}" class="mkt-surface p-8 rounded-[2.5rem] border mkt-border shadow-2xl relative overflow-hidden animate-in fade-in duration-500">
    
    {{-- Tabs Navigation --}}
    <div class="flex items-center gap-2 mb-8 bg-slate-100 dark:bg-slate-900/50 p-1.5 rounded-2xl w-fit border mkt-border">
        <button wire:click="setTab('calculator')" 
            class="px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $activeTab === 'calculator' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-600/20' : 'mkt-text-muted hover:mkt-text' }}">
            Calculator
        </button>
        <button wire:click="setTab('history')" 
            class="px-6 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all {{ $activeTab === 'history' ? 'bg-blue-600 text-white shadow-lg shadow-blue-600/20' : 'mkt-text-muted hover:mkt-text' }}">
            Riwayat Quotation
        </button>
    </div>
    ...
    @if($activeTab === 'calculator')
        <div class="animate-in fade-in slide-in-from-left-4 duration-500">
            {{-- Header --}}
            <div class="flex items-center justify-between mb-8 pb-6 border-b mkt-border">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-emerald-600 rounded-2xl flex items-center justify-center text-white text-2xl shadow-lg shadow-emerald-600/20">
                        🧮
                    </div>
                    <div>
                        <h3 class="text-xl font-black uppercase tracking-tighter mkt-text leading-tight italic">HPP & Price Calculator</h3>
                        <p class="text-[10px] font-bold mkt-text-muted uppercase tracking-widest">Simulasi Biaya Produksi & Harga Jual</p>
                    </div>
                </div>
                <div class="text-right">
                    <span class="bg-emerald-600/10 text-emerald-600 text-[10px] px-4 py-1.5 rounded-full font-black italic">REAL-TIME SIMULATION</span>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-10">
                ...
                <div class="bg-slate-900 rounded-[2.5rem] p-8 text-white shadow-2xl relative overflow-hidden flex flex-col justify-between border border-white/5">
                    <div class="absolute top-0 right-0 w-40 h-40 bg-emerald-600/10 rounded-full -mr-20 -mt-20 blur-3xl"></div>
                    
                    <div class="relative z-10">
                        <h4 class="text-xs font-black uppercase tracking-widest text-emerald-400 mb-8 flex items-center gap-2 italic">
                            <span class="w-4 h-1 bg-emerald-400"></span> Calculation Result
                        </h4>

                        <div class="space-y-8">
                            <div>
                                <p class="text-[9px] font-black uppercase text-slate-400 tracking-widest mb-1">HARGA POKOK PRODUKSI (MODAL)</p>
                                <h2 class="text-3xl font-black text-white italic tracking-tighter leading-none">Rp {{ number_format($hpp, 0, ',', '.') }} <span class="text-xs text-slate-500">/kg</span></h2>
                            </div>

                            <div class="bg-white/5 p-6 rounded-3xl border border-white/10">
                                <p class="text-[9px] font-black uppercase text-emerald-400 tracking-widest mb-1">REKOMENDASI HARGA JUAL (NETT)</p>
                                <h2 class="text-4xl font-black text-emerald-400 italic tracking-tighter leading-none">Rp {{ number_format($selling_price, 0, ',', '.') }}</h2>
    ...
    @if($activeTab === 'history')
        <div class="animate-in fade-in slide-in-from-right-4 duration-500">
            <div class="flex items-center justify-between mb-8 pb-6 border-b mkt-border">
                <div class="flex items-center gap-4">
                    <div class="w-12 h-12 bg-blue-600 rounded-2xl flex items-center justify-center text-white text-2xl shadow-lg shadow-blue-600/20">
                        📜
                    </div>
                    <div>
                        <h3 class="text-xl font-black uppercase tracking-tighter mkt-text leading-tight italic">Saved Quotations</h3>
                        <p class="text-[10px] font-bold mkt-text-muted uppercase tracking-widest">Daftar Penawaran Harga Yang Pernah Disimpan</p>
                    </div>
                </div>
            </div>
