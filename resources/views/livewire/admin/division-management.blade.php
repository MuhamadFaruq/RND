<div class="min-h-screen w-full mkt-bg mkt-text font-sans italic flex flex-col transition-colors duration-300">
    <div class="p-4 md:p-8 flex-grow container mx-auto">
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-10 border-b mkt-border pb-8 gap-6 text-left">
            ...
            <button wire:click="openModal" class="bg-indigo-600 text-white px-8 py-3 rounded-2xl font-black uppercase italic shadow-2xl hover:bg-indigo-700 transition transform hover:scale-105">
                + UNIT BARU
            </button>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
            @forelse($divisions as $div)
                <div class="mkt-surface p-5 md:p-8 rounded-[2rem] md:rounded-[3rem] border mkt-border shadow-2xl flex justify-between items-center group hover:border-indigo-600 transition-all duration-300 text-left animate-in fade-in duration-300">
                    ...
                    <div class="flex gap-2 opacity-100 md:opacity-0 md:group-hover:opacity-100 transition-opacity duration-300 shrink-0">
                        <button wire:click="edit({{ $div->id }})" class="p-2.5 md:p-3 mkt-surface mkt-text rounded-xl md:rounded-2xl hover:bg-indigo-600 hover:text-white transition-colors border mkt-border shadow-sm flex items-center justify-center" title="Edit Unit">
        ...
        <div class="fixed inset-0 z-[110] flex items-center justify-center bg-black/90 backdrop-blur-sm p-4">
            <div class="mkt-surface border mkt-border p-10 rounded-[2.5rem] max-w-lg w-full text-center shadow-2xl animate-in zoom-in duration-300">
                <div class="w-24 h-24 bg-indigo-600/10 border-4 border-indigo-600/20 text-indigo-600 rounded-full flex items-center justify-center mx-auto mb-8">
                ...
                <p class="mkt-text-muted text-sm font-bold uppercase mb-10 leading-relaxed tracking-wide italic">
                    Apakah Anda yakin ingin menghapus unit <span class="text-indigo-600">{{ $selectedDivisionName }}</span>?<br>
                    Data ini akan dihapus permanen dari sistem RND.
                </p>
                
                <div class="flex flex-col sm:flex-row gap-4 justify-center">
                    <button wire:click="delete({{ $selectedDivisionId }})" 
                        class="bg-indigo-600 text-white px-10 py-4 rounded-2xl font-black uppercase italic shadow-xl shadow-indigo-900/40 hover:bg-black transition-all transform hover:scale-105">
                        YA, HAPUS DATA
                    </button>
                ...
                <div class="relative w-full max-w-md transform overflow-hidden rounded-[2.5rem] mkt-surface border mkt-border shadow-2xl transition-all">
                    <div class="p-10 text-left">
                        <h3 class="text-3xl font-black uppercase italic mb-8 mkt-text tracking-tighter">
                            {{ $divisionId ? 'Update' : 'Register' }} <span class="text-indigo-600">Unit</span>
                        </h3>
                        <form wire:submit.prevent="save" class="space-y-6">
                            <div>
                                <label class="block text-[10px] font-black uppercase text-slate-500 mb-2 tracking-[0.2em]">Nama Divisi / Unit</label>
                                <input type="text" wire:model="name" class="w-full mkt-input border mkt-border rounded-2xl px-5 py-4 text-lg font-black mkt-text focus:border-indigo-600 focus:ring-0 uppercase italic placeholder:mkt-text-muted">
                                @error('name') <span class="text-indigo-500 text-[10px] font-bold mt-2 block uppercase italic">{{ $message }}</span> @enderror
                            </div>
                            <div>
                                <label class="block text-[10px] font-black uppercase text-slate-500 mb-2 tracking-[0.2em]">Deskripsi Operasional</label>
                                <textarea wire:model="description" class="w-full mkt-input border mkt-border rounded-2xl px-5 py-4 text-sm font-bold mkt-text focus:border-indigo-600 focus:ring-0 placeholder:mkt-text-muted" rows="3"></textarea>
                            </div>
                            <div class="flex flex-row-reverse gap-4 pt-4">
                                <button type="submit" class="bg-indigo-600 text-white px-10 py-3 rounded-xl font-black uppercase italic hover:bg-indigo-700 transition">SIMPAN UNIT</button>
